...
| **refreshPriority** | Number | Higher values refresh first. Use when creating ScrollTriggers in non–top-to-bottom order: assign priorities so triggers still refresh in page order. |
...
- ✅ Create ScrollTriggers in the order they appear on the page (top to bottom, scroll 0 → max). When they are created in a different order (e.g. dynamic or async), use higher **refreshPriority** values for triggers that must refresh earlier so the effective order remains top-to-bottom.

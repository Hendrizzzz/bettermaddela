...
Let the browser manage compositing by default. Use **will-change** only after profiling demonstrates a real benefit. Apply it narrowly shortly before the animation and restore **auto** after the animation so promoted layers do not retain unnecessary memory.
...
- ✅ Animate **transform** and **opacity**; use **will-change** only after measurement, narrowly and temporarily, then restore **auto**.
...
- ❌ Leave **will-change** or **force3D** on elements “just in case”; retained compositor layers can waste memory, especially on constrained devices.

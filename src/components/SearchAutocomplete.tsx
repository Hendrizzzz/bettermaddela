...
const SearchAutocomplete = forwardRef<SearchAutocompleteHandle, Props>(
  function SearchAutocomplete({ placeholder }, ref) {
          ...
          {categories.length > 0 && (
            <div className="search-filters">
              <button
                type="button"
                data-category=""
                className={`search-filter-btn ${!currentCategory ? 'active' : ''}`}
              ...
              {categories.slice(0, 5).map((cat) => (
                <button
                  ...
                  className={`search-filter-btn ${currentCategory === cat.id ? 'active' : ''}`}
    ...
    return (
      <div ref={wrapperRef} className="search-autocomplete-wrapper">
        <input
          ref={inputRef}
          type="search"
          id="hero-search"
